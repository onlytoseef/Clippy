...
export function TopNavDashboard({ children, activeTab, onTabChange }: TopNavDashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      {/* Top Navigation Bar */}
      <div className="flex justify-center pt-2 md:pt-3 px-2 sm:px-4">
        <div className="bg-white/20 dark:bg-gray-800/20 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 rounded-full shadow-xl">
          <div className="flex items-center justify-center px-3 sm:px-6 md:px-8 py-1.5 sm:py-2">
              ...
              {navigation.map((item) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`relative flex flex-col items-center justify-center p-1 sm:p-1.5 rounded-lg sm:rounded-xl transition-all duration-200 ${
                      isActive 
                    ...
                    whileTap={{ scale: 0.95 }}
                  >
                    <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${isActive ? 'text-blue-600 dark:text-blue-400' : ''}`} />
                    <span className="text-[10px] sm:text-xs mt-0.5 font-medium hidden xs:block sm:block">{item.name}</span>
                    
                    {/* Active Indicator */}
                    {isActive && item.id !== 'account' && (
                      <motion.div
                        layoutId="activeTab"
                        className="absolute -bottom-0.5 left-1/2 transform -translate-x-1/2 w-5 sm:w-6 h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full"
                        initial={false}
                    ...
                    {item.id === 'account' && showProfileDropdown && (
                      <motion.div
                        className="absolute -bottom-0.5 left-1/2 transform -translate-x-1/2 w-5 sm:w-6 h-0.5 bg-gray-600 dark:bg-gray-400 rounded-full"
                        initial={false}
                        transition={{ type: "spring", stiffness: 500, damping: 30 }}
                      />
                    )}
